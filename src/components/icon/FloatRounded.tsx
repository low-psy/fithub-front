...
const FloatRoundedIcon: React.FC<IconComponentProps> = ({
  ...
  return (
    ...
    >
      <span
        className={`material-symbols-rounded  ${isStarFilled ? 'filled' : 'unfilled'}  font-main text-5xl sm:text-4xl ${iconColor || 'text-main'} transition-transform duration-150 ${isClicked ? 'scale-125' : 'scale-100'}  `}
      >
        {children}
      </span>
    </button>
