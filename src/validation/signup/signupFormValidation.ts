...
export const validateAdditionalInfoForm = (
  password: string,
...
export const validateSocialSignupForm = (
  email: string,
  name: string,
  phone: string,
  setErrorMsg: React.Dispatch<React.SetStateAction<ISocialSignupErrorMsg>>,
) => {
  const emailValidation = validateEmail(email);
  if (emailValidation.status === false) {
    setErrorMsg({ ...defaultErrorMsg, email: emailValidation.errorMessage });
  ...
  const nameValidation = validateName(name);
  if (nameValidation.status === false) {
    setErrorMsg({ ...defaultErrorMsg, name: nameValidation.errorMessage });
    return false;
  }
